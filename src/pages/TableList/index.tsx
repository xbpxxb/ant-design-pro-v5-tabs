...
const TableList: React.FC = () => {
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        onClick={() => {
          history.push({
            pathname: '/detail',
            state: {aaa: new Date().getTime()},
            query: {bbb: `${new Date().getTime()}` }
          })
        }}
      >
        详情
  ...
  const handleBeforeSearchSubmit = (params: {name?: string, newName?: string}) => {
    console.log(params)
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem>
        ...
        dataSource={[{key: 1}]}
        onRequestError={(err) => {}}
        beforeSearchSubmit={(params) => handleBeforeSearchSubmit(params)}
        columns={columns}
        rowSelection={{
